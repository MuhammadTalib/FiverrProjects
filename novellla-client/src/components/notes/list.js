...
function NotesList() {
  ...
  let handleToggleNoteButton = (list, index, num) => {
    // console.log("Toggle",list, index, num)
    let icons = null;
  ...
  useEffect(() => {
    let handleMoveToSubNotes = (from, to) => {
      console.log("handleMoveToSubNotes",handleMoveToSubNotes)
      // notes
    ...
    let handleMoveToMainNotes = (from, id, target) => {
      console.log("handleMoveToMainNotes",from, id, target)
    ...
    let handleMovePosition = (from, to) => {
      console.log("handleMovePosition",from , to)
      // notes
      let notes = [...state.notes.lists];
    ...
    let handleMoveSubNotePosition = (group, from, to) => {
      console.log("handleMoveSubNotePosition",group, from, to)
      if (from === to) return;
    ...
    $('.notes-list > div').sortable({
      ...
      sort: function (event, ui) {
        ...
        let subNotes = $(ui.item)
          ...
          $("sort").removeClass("ui-sortable-handle");
